...
*Shipped 2026-07-08. Patch release (v1.2.0 → v1.2.1) **form**; force-re-released under the same tag after the `--embed-in-tags` opt-in tag-embedding work and the 2-EXE single dual-mode build refactor landed. Read together with the `v1.2.0` release above for the full picture.*

> **Tag-conflict advisory.** The pre-existing `v1.2.1` tag on `origin/main` is repointed to this commit; the tag SHA moves on the next `git push --force origin v1.2.1`. Existing pinned `git checkout v1.2.1` references fast-forward / rebase to the new SHA on `git pull`. Confirm with `git log --oneline v1.2.1` -- the canonical notes for this tag live in this file.
...
This release also bundles `v1.2.1`-tagged work that shipped in two
earlier force-rolled iterations (the `--embed-in-tags` opt-in tag
embedding and the 2-EXE single dual-mode build refactor), consolidated
under one `v1.2.1` tag to keep the git-tag chain aligned with what
`build.bat` ships:
* **`--embed-in-tags`** opt-in write of a plain-text lyrics tag into
  the audio file's native metadata (per-format dispatch: `USLT` /
  Vorbis `LYRICS` / MP4 `©lyr` / `WM/Lyrics` / APEv2 `LYRICS`; WAV/TAK
  /raw AAC skipped cleanly with a `[tags=skipped (no standard tag)]`
  log suffix).
* **2-EXE build pipeline** — each variant ships as a single dual-mode
  `.exe` (instead of separate CLI/GUI pairs); empty argv / `--gui`
  pops the Tk GUI, terminal argv runs the CLI loop. The portable
  build bakes `models/whisper-small/` (~500 MB) by default with a
  backward-compat fallback to `models/whisper-base/`.

Both items are detailed in the new sub-sections below.
...
### Build: 2-EXE single dual-mode binary per variant

Each build variant now ships as ONE dual-mode `.exe` instead of
a separate CLI/GUI pair (the four-exe layout shrinks to two):

* **Distinguishing surface = argv.** `lyricsfag._wants_gui()` returns
  `True` for empty argv (double-click → GUI) or explicit `--gui` /
  `-g` / `--gui=1`. Any other argv (positional path or any flag)
  drives the CLI loop. Same `.exe`, same entry point
  (`lyricsfag.py`), no second binary to ship.
* **Portable bundle: `models/whisper-small/` (~500 MB) by default.**
  `build-portable.bat` auto-detects `models/whisper-base/` as a
  backward-compat fallback for v1.2.x dev trees (no re-seed
  required). The `requirements-audio.txt` extra is still untouched
  by `build-lite.bat`, so the lite `.exe` keeps the documented
  ~50 MB footprint without bundling `torch`.
* **`--collect-all faster_whisper`** replaces the older
  `--hidden-import faster_whisper` for parity with the existing
  `--collect-all demucs` — the bundled `.exe` now carries all of
  faster-whisper's runtime data + CTranslate2 `.so` files.

### Feature: `--embed-in-tags` opt-in tag embedding

A new `--embed-in-tags` (CLI) / "Embed lyrics in tags" (GUI) flag
writes a plain-text lyrics tag into each audio file's native
metadata, dispatching by format:

* ID3 `USLT` (UTF-8, lang='eng') for MP3
* Vorbis `LYRICS` for FLAC/OGG-Vorbis/Opus
* MP4 atom `©lyr` for M4A
* ASF descriptor `WM/Lyrics` for WMA
* APEv2 `LYRICS` for WavPack/APE
* WAV / TAK / raw AAC → skipped cleanly with a `[tags=skipped (no
  standard tag)]` log suffix; never a batch failure

The `.lrc` keeps the synced / header-rich view; the tag holds only
the lyric text. `--force` overwrites; `--no-force` honours any
pre-existing tag the same way it does for the `.lrc` sidecar.
Tag-failure does **not** downgrade the `.lrc` outcome — the sidecar
is already on disk and you get a `[tags=ERROR: ...]` suffix, not
a red fail line.

The dispatch lives in the new `lyricsfag_lib/tags.py` module —
`embed_lyrics(path, plain_text, force=False)` returns an
`EmbedResult` dataclass (status, format, tag_name, error) so
the CLI / GUI can render the `[tags=...]` log suffix without
inspecting exceptions themselves.
...
* `RELEASE_NOTES.md`: prepended the bilingual `## v1.2.1`
  ...
  (`RELEASE_PROCESS.md`); force-re-released under the same
  `v1.2.1` tag after `--embed-in-tags` + 2-EXE build refactor
  landed.
* **2-EXE build pipeline + `--embed-in-tags` (consolidated under
  this same `v1.2.1` tag):**
  * `lyricsfag.py`: new module-level `_GUI_FLAG_RE` regex
    (`^(?:--?gui|-g)(?:=|$)`) and `_wants_gui(argv)` helper;
    `main()` dispatches to `lyricsfag_gui.main()` for empty argv
    or an explicit GUI flag, so one dual-mode `.exe` covers both
    surfaces. New `--embed-in-tags` CLI flag routes through
    `lyricsfag_lib.tags.embed_lyrics`.
  * `lyricsfag_gui.py`: launches cleanly under the no-arg /
    `--gui` dual-mode dispatch (its own `main()` is invoked from
    `lyricsfag.main()` when `_wants_gui` returns True); "Embed
    lyrics in tags" tickbox added; no widget surface breakage.
  * `lyricsfag_lib/tags.py`: **new file**, opt-in tag-embed
    dispatch (`embed_lyrics(path, plain_text, force=False)` →
    `EmbedResult` dataclass; per-format dispatch: ID3 `USLT`,
    Vorbis `LYRICS`, MP4 `©lyr`, ASF `WM/Lyrics`, APEv2
    `LYRICS`).
  * `lyricsfag_lib/audio_analysis.py`: `_BUNDLED_MODEL_DIR`
    replaced by tuple `_BUNDLED_MODEL_DIRS` walked in priority
    order (`models/whisper-small/` first,
    `models/whisper-base/` as the backward-compat fallback).
    Portable build bakes `models/whisper-small/` by default.
  * `scripts/download_whisper_model.py`: `DEFAULT_SIZE` /
    `DEFAULT_OUTPUT_DIR` flipped from `base` /
    `models/whisper-base/` to `small` /
    `models/whisper-small/` (the portable build's baked default).
  * `build.bat` / `build-lite.bat` / `build-portable.bat`: each
    variant now produces ONE dual-mode `.exe`; the legacy
    `LyricsFAG-GUI-Lite.exe` / `LyricsFAG-GUI-Portable.exe`
    artifacts are gone. Header docstrings + `:summary` section
    rewritten for the single binary layout.
  * `README.md` / `README.ru.md`: badge / tool-stamp examples /
    per-release summary / build sections updated for the 2-EXE
    build, the `[tool:LyricsFAG 1.2.1]` producer credit, and the
    `--embed-in-tags` opt-in switch.
...
*Выпущено 2026-07-08. Patch-релиз (v1.2.0 → v1.2.1) **формы**; force-re-release под тем же тегом после landing'а `--embed-in-tags` opt-in записи тегов и 2-EXE single dual-mode рефакторинга билд-пайплайна. Читайте вместе с `v1.2.0` релизом выше для полной картины.*

> **Tag-conflict advisory.** Пре-существующий тег `v1.2.1` на `origin/main` repointed на этот коммит; SHA тега смещается на следующем `git push --force origin v1.2.1`. Существующие pinned `git checkout v1.2.1`-ссылки fast-forward / rebase на `git pull`. Подтвердите через `git log --oneline v1.2.1` -- канонические заметки для этого тега живут в этом файле.
...
Этот релиз также объединяет под одним тегом `v1.2.1` ещё две итерации,
которые шипнулись принудительно-форс-роллнуто (opt-in запись тегов
`--embed-in-tags` и 2-EXE dual-mode рефакторинг билд-пайплайна), чтобы
git-tag-цепочка оставалась синхронизированной с тем, что
выпускает `build.bat`:
* **`--embed-in-tags`** -- opt-in запись plain-text тега текста песни
  в нативные метаданные аудиофайла (диспатч по формату: `USLT` /
  Vorbis `LYRICS` / MP4 `©lyr` / `WM/Lyrics` / APEv2 `LYRICS`;
  WAV/TAK/сырой AAC тихо пропускаются в логе как `[tags=skipped (no
  standard tag)]`).
* **2-EXE билд-пайплайн** -- каждый вариант поставляется как один
  dual-mode `.exe` (вместо пары CLI/GUI); пустой argv / `--gui` открывает
  Tk-GUI, терминальный argv запускает CLI-цикл. Portable-сборка вшивает
  `models/whisper-small/` (~500 МБ) по умолчанию с backward-compat
  фолбэком на `models/whisper-base/`.

Оба пункта детализированы в новых под-секциях ниже.
...
### Сборка: 2-EXE один dual-mode `.exe` на вариант

Каждый вариант теперь поставляется как один dual-mode `.exe` (вместо
пары CLI/GUI; четыре exe схлопываются в два):

* **Отличие — argv.** `lyricsfag._wants_gui()` возвращает `True` для
  пустого argv (двойной клик → GUI) или явного `--gui` / `-g` /
  `--gui=1`. Любой другой argv (позиционный путь или флаг) -- CLI-цикл.
  Тот же `.exe`, та же точка входа (`lyricsfag.py`), никакого второго
  бинаря.
* **Portable-бандл: `models/whisper-small/` (~500 МБ) по умолчанию.**
  `build-portable.bat` авто-детектит `models/whisper-base/` как
  backward-compat фолбэк для v1.2.x dev-деревьев (re-seed не
  требуется). Доп. файл `requirements-audio.txt` по-прежнему не
  трогается `build-lite.bat`, так что lite-`.exe` сохраняет
  задокументированные ~50 МБ без бандла `torch`.
* **`--collect-all faster_whisper`** заменяет старый
  `--hidden-import faster_whisper` для паритета с уже-имеющимся
  `--collect-all demucs` -- вшитый `.exe` теперь несёт все
  runtime-данные и CTranslate2 `.so`-файлы faster-whisper.

### Feature: `--embed-in-tags` opt-in запись тегов

Новый `--embed-in-tags` (CLI) / "Embed lyrics in tags" (GUI) пишет
plain-text тег текста песни в нативные метаданные аудиофайла;
диспатч по формату:

* ID3 `USLT` (UTF-8, lang='eng') для MP3
* Vorbis `LYRICS` для FLAC/OGG-Vorbis/Opus
* атом MP4 `©lyr` для M4A
* ASF-дескриптор `WM/Lyrics` для WMA
* APEv2 `LYRICS` для WavPack/APE
* WAV / TAK / сырой AAC → тихо пропускается в логе как `[tags=skipped
  (no standard tag)]`; батч не падает

`.lrc` сохраняет синхронизированный / расширенный-заголовками вид;
в тег попадает только сам текст. `--force` перезаписывает;
`--no-force` оставляет существующий тег в покое, как и `.lrc`.
Сбой записи тега **не** даунгрейдит `.lrc`-статус -- sitecar уже
на диске (в логе будет `[tags=ERROR: ...]`, не красный fail).

Диспатч живёт в новом модуле `lyricsfag_lib/tags.py` --
`embed_lyrics(path, plain_text, force=False)` возвращает dataclass
`EmbedResult` (status, format, tag_name, error), так что CLI / GUI
рендерят суффикс `[tags=...]` в логе без инспектирования исключений.
...
* `RELEASE_NOTES.md`: добавлена в начало двуязычная
  ...
  (`RELEASE_PROCESS.md`); force-re-released под тем же тегом
  `v1.2.1` после landing'а `--embed-in-tags` + 2-EXE-рефактора
  сборки.
* **2-EXE билд-пайплайн + `--embed-in-tags` (объединено под этим
  же тегом `v1.2.1`):**
  * `lyricsfag.py`: новый модульный regex `_GUI_FLAG_RE`
    (`^(?:--?gui|-g)(?:=|$)`) и хелпер `_wants_gui(argv)`;
    `main()` диспатчит в `lyricsfag_gui.main()` на пустом argv
    или явном GUI-флаге -- один dual-mode `.exe` обслуживает
    обе поверхности. Новый CLI-флаг `--embed-in-tags` идёт через
    `lyricsfag_lib.tags.embed_lyrics`.
  * `lyricsfag_gui.py`: чисто стартует под no-arg / `--gui`
    dual-mode-диспатч (его собственный `main()` зовётся из
    `lyricsfag.main()`, когда `_wants_gui` возвращает True);
    добавлен чекбокс "Embed lyrics in tags"; без изменений
    виджетной поверхности.
  * `lyricsfag_lib/tags.py`: **новый файл**, opt-in-диспатч
    записи тегов (`embed_lyrics(path, plain_text, force=False)`
    → dataclass `EmbedResult`; per-format диспатч: ID3 `USLT`,
    Vorbis `LYRICS`, MP4 `©lyr`, ASF `WM/Lyrics`, APEv2
    `LYRICS`).
  * `lyricsfag_lib/audio_analysis.py`: `_BUNDLED_MODEL_DIR`
    заменён кортежем `_BUNDLED_MODEL_DIRS`, обход в порядке
    приоритета (`models/whisper-small/` первый,
    `models/whisper-base/` как backward-compat фолбэк).
    Portable-сборка вшивает `models/whisper-small/` по
    умолчанию.
  * `scripts/download_whisper_model.py`: `DEFAULT_SIZE` /
    `DEFAULT_OUTPUT_DIR` перекинуты с `base` /
    `models/whisper-base/` на `small` /
    `models/whisper-small/` (запекаемый по умолчанию размер
    portable-сборки).
  * `build.bat` / `build-lite.bat` / `build-portable.bat`:
    каждый вариант теперь даёт ОДИН dual-mode `.exe`;
    legacy-артефакты `LyricsFAG-GUI-Lite.exe` /
    `LyricsFAG-GUI-Portable.exe` больше не выпускаются.
    Заголовочные docstring'и + `:summary`-секция переписаны под
    одно-бинарный лейаут.
  * `README.md` / `README.ru.md`: бейдж / примеры tool-stamp /
    per-release summary / секции сборки обновлены под
    2-EXE-сборку, `[tool:LyricsFAG 1.2.1]`-кредит продюсера и
    `--embed-in-tags` opt-in переключатель.
