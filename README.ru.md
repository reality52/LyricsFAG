...
4. *(Опционально)* Перед первым запуском сохраните локальные веса
   ...
   REM Веса Demucs для дефолтного 'htdemucs_ft' (~84 MB, одна модель).
   ...
   *(Если вы явно включили старый 5-модельный бэг через
   ``--demucs-model htdemucs``, скачивайте именно его:
...
REM 2. Веса Demucs для дефолтного 'htdemucs_ft' (~84 MB, одна модель).
...
REM    --model htdemucs вместо дефолтного — это тянет крупный 5-модельный бэг.)
...
Подробные заметки к релизам — единый [`RELEASE_NOTES.md`](RELEASE_NOTES.md) в корне репозитория (все поставленные версии в одном файле, newest-first).

### v1.1.0

Изменение контракта аудио-фолбэка + новый локальный источник `audio`.

- **Demucs теперь обязателен** при включённом аудио-фолбэке.
  Переключатель on/off `Demucs` в аудио-панели убран: стадия
  вокальной изоляции Demucs всегда работает вместе с
  `--use-audio-analysis` (или если в GUI стоит соответствующая
  галка). Чтобы полностью отключить локальный фолбэк — снимите
  `Use audio analysis` (GUI) или уберите `--use-audio-analysis`
  (CLI). CLI-флаг `--no-demucs` удалён; раскладка весов Demucs и
  дефолтный `htdemucs_ft` не поменялись.
- **Новый источник `audio`** (в GUI выпадающий Source + CLI
...
- **Шорткат по имени файла** для аудио-ветки. Треки, в имени файла
  (без учёта регистра и пунктуации) содержащие `instrumental`,
  `karaoke`, `off vocal`, `no vocal`, `minus one`, `backing track` и
  т.п., никогда не отправляются в Whisper. Ветки LRCLIB / Genius
  по-прежнему пытаются первыми (оригинал караоке-файла часто
  реальная песня с реальным текстом) — шорткат действует только
  на аудио-ветку.
- **Обратная совместимость:** `settings.json` от v1.0.x с ключом
  `demucs` теперь молча игнорируется вместо падения GUI на старте
  (раньше код ссылался на `self.demucs_var`, которого больше нет).
- `lyricsfag_lib.__version__` теперь `1.1.0`.

Подробности — в [`RELEASE_NOTES.md`](RELEASE_NOTES.md) (запись v1.1.0).

### v1.0.2

Полировка перед релизом. API не меняется.

- **GUI:** кнопка `?  Help` (последняя в ряду тулбара, правее Open output
  folder) открывает `messagebox.showinfo(...)` с цепочкой
  LRCLIB → Genius → локальное аудио, подсказкой про `GENIUS_ACCESS_TOKEN` / `--genius-token`, размерами
  Whisper / Demucs при первом запуске и советом «сначала через `--dry-run`
  на копии». (В v1.1.0 в цепочку добавился источник `audio`; попап по-прежнему описывает ту же трёхступенчатую
  последовательность `LRCLIB → Genius → локальное аудио`, потому что
  для `source=auto` этот порядок не менялся.)
  / `--genius-token`, размерами Whisper / Demucs при первом запуске и
  советом «сначала через `--dry-run` на копии».
- **GUI:** тултипы на всех основных интерактивных элементах (entries,
  чекбоксы, комбобоксы, кнопки) — задержка hover 500 мс, автоскрытие 8 с.
- **GUI:** финальный попап — Done / Stopped / worker-crashed,
  страховка гонки с `_on_close` через `winfo_exists()`. Пустые папки
  больше не открывают его.
- **Старт:** `audio_analysis.describe_models_layout()` логирует
  резолвящиеся пути `models/whisper-base/` + `models/demucs/` рядом
  со скриптом, чтобы пользователь без grep'а по исходникам видел,
  куда встанут веса.
- **Cleanup:** убраны неиспользуемая константа `COLOUR_BG` и
  неиспользуемый импорт `_format_provider_breakdown`; аудио-лейбл
  Device переименован, чтобы не перекрывать бейдж статуса.
  `lyricsfag_lib.__version__` теперь `1.0.2` (раньше залипало на `0.1.0`
  со времён самого первого скелета).

Подробности — в [`RELEASE_NOTES.md`](RELEASE_NOTES.md) (запись v1.0.2).

### v1.0.1

Первый отмеченный релиз — полировка README, перевод на русский
(`README.ru.md`), AI-дисклеймер, упрощённый Whisper-диагностик в `.lrc`.

Подробности — в [`RELEASE_NOTES.md`](RELEASE_NOTES.md) (запись v1.0.1; файл
добавлен на `main` уже в v1.0.2 как ретроактивный бэкфилл; в дереве
git-тега `1.0.1` его нет).
